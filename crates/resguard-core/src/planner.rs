use crate::profile::{Class, Profile};
...
pub fn build_apply_plan(profile: &Profile, root: &Path, options: &PlanOptions) -> Vec<Action> {
    ...
    dirs.insert(system_dropin.parent().unwrap().to_path_buf());
    dirs.insert(user_dropin.parent().unwrap().to_path_buf());
...
fn collect_classes(profile: &Profile) -> BTreeMap<String, &Class> {
...
pub fn render_class_slice(name: &str, class: &Class, user: bool, no_oomd: bool) -> String {
    let mut s = String::new();
    s.push_str(MANAGED_HEADER);
    s.push_str("\n[Unit]\n");
    if user {
